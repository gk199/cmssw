...
void l1t::Stage2Layer2JetAlgorithmFirmwareImp1::create(const std::vector<l1t::CaloTower>& towers,
  ...
  for (int etaSide = 1; etaSide >= -1; etaSide -= 2) {
    ...
    for (int i = 1; i <= CaloTools::mpEta(CaloTools::kHFEnd); i++) {
      if (!((i - 1) % 4))
        ringGroup1.push_back(i * etaSide);
      else if (!((i - 2) % 4))
        ringGroup2.push_back(i * etaSide);
      else if (!((i - 3) % 4))
        ringGroup3.push_back(i * etaSide);
      else if (!((i - 4) % 4))
        ringGroup4.push_back(i * etaSide);
    }
    ...
    for (unsigned ringGroupIt = 1; ringGroupIt <= theRings.size(); ringGroupIt++) {
	  ...
	  if ((iDelay > 0) && (abs(ieta) < 29)) std::cout << "in Stage2Layer2JetAlgorithmFirmwareImp iDelay from hwQual = " << iDelay << " at ieta, iphi = " << ieta << ", " << iphi << std::endl;
          ...
          if (iEt < floor(params_->jetSeedThreshold() / params_->towerLsbSum()))
            continue;

          if (seedEt == CaloTools::kSatHcal || seedEt == CaloTools::kSatEcal || seedEt == CaloTools::kSatTower)
            satSeed = true;
          ...
          for (int deta = -4; deta < 5; ++deta) {
            for (int dphi = -4; dphi < 5; ++dphi) {
              ...
              // wrap around phi
              while (iphiTest > CaloTools::kHBHENrPhi)
                iphiTest -= CaloTools::kHBHENrPhi;
              while (iphiTest < 1)
                iphiTest += CaloTools::kHBHENrPhi;
	      ...
	      //	      if ((towDelay > 0) && (abs(ietaTest) < 29)) std::cout<< "in Stage2Layer2JetAlgorithmFirmwareImp towDelay from hwQual = " << towDelay << " at ieta, iphi = " << ietaTest << ", " << iphiTest << std::endl;
              ...
              if (mask_[8 - (dphi + 4)][deta + 4] == 0)
                continue;
              else if (mask_[8 - (dphi + 4)][deta + 4] == 1)
                vetoCandidate = (seedEt < towEt);
              else if (mask_[8 - (dphi + 4)][deta + 4] == 2)
                vetoCandidate = (seedEt <= towEt);
	  ...
	  //	  if (iDelay != 0) std::cout << "in Stage2Layer2JetAlgorithmFirmwareImp iDelay = " << iDelay << " at ieta, iphi = " << ieta << ", " << iphi << std::endl;
          ...
          if (!vetoCandidate) {
            ...
            if (!params_->jetBypassPUS() && !satSeed) {
              ...
              if (PUSubMethod == "PhiRing1") {
                ...
                if (jetEta > 0 && jetEtaLow <= 0)
                  jetEtaLow -= 1;
                if (jetEta < 0 && jetEtaHigh >= 0)
                  jetEtaHigh += 1;
                puEt = 0;
                for (int ieta = jetEtaLow; ieta <= jetEtaHigh; ++ieta) {
                  if (ieta == 0)
                    continue;  // eta start from +- 1
...
int l1t::Stage2Layer2JetAlgorithmFirmwareImp1::donutPUEstimate(int jetEta,
  ...
  while (iphiUp > CaloTools::kHBHENrPhi)
    iphiUp -= CaloTools::kHBHENrPhi;
  int iphiDown = jetPhi - size;
  while (iphiDown < 1)
    iphiDown += CaloTools::kHBHENrPhi;
  ...
  for (int ieta = jetEta - size + 1; ieta < jetEta + size; ++ieta) {
    if (abs(ieta) > CaloTools::mpEta(CaloTools::kHFEnd) || abs(ieta) < 1)
      continue;
    int towerEta;
  ...
  for (int iphi = jetPhi - size + 1; iphi < jetPhi + size; ++iphi) {
    int towerPhi = iphi;
    while (towerPhi > CaloTools::kHBHENrPhi)
      towerPhi -= CaloTools::kHBHENrPhi;
    while (towerPhi < 1)
      towerPhi += CaloTools::kHBHENrPhi;
...
std::vector<int> l1t::Stage2Layer2JetAlgorithmFirmwareImp1::getChunkyRing(l1t::Jet& jet,
  ...
  int ringSize = 4;
  if (chunkyString == "ChunkySandwich8")
    ringSize = 8;
  ...
  for (int stripIt = 0; stripIt < nStrips; stripIt++) {
    ...
    for (int ieta = jetEta - size + 1; ieta < jetEta + size; ++ieta) {
      if (abs(ieta) > CaloTools::mpEta(CaloTools::kHFEnd))
        continue;

      int towEta = ieta;
      if (jetEta > 0 && towEta <= 0)
        towEta -= 1;
      if (jetEta < 0 && towEta >= 0)
        towEta += 1;
    ...
    if (chunkyString == "ChunkyDonut") {
      ...
      if (jetEta < 0 && ietaUp >= 0)
        ietaUp += 1;
      if (jetEta > 0 && ietaDown <= 0)
        ietaDown -= 1;
      ...
      for (int iphi = jetPhi - size + 1; iphi < jetPhi + size; ++iphi) {
        if (abs(ietaUp) <= CaloTools::mpEta(CaloTools::kHFEnd)) {
          int towPhi = iphi;
          while (towPhi > CaloTools::kHBHENrPhi)
            towPhi -= CaloTools::kHBHENrPhi;
          while (towPhi < 1)
            towPhi += CaloTools::kHBHENrPhi;
    ...
    } else if (chunkyString == "ChunkySandwich2Ave" ||
               chunkyString == "ChunkySandwich") {  //simplest of the sandwiches - we will just reuse phi flaps
      ring[2] = ((ring[0] + ring[1]) >> 1);         //bitwise, effective div by 2
      ring[3] = (ring[0] + ring[1]);                // just make sure it is >= max val
    } else if (chunkyString == "ChunkySandwich4" ||
               chunkyString ==
                   "ChunkySandwich8") {  //Need to calculate two or siz additional phiflaps. Note that ChunkySandwich defaults to ChunkySandwich4 variant
      ...
      for (int rI = 0; rI < (ringSize - 2) / 2; ++rI) {
        ...
        while (iphiUp2 > CaloTools::kHBHENrPhi)
          iphiUp2 -= CaloTools::kHBHENrPhi;
        while (iphiDown2 < 1)
          iphiDown2 += CaloTools::kHBHENrPhi;
...
int l1t::Stage2Layer2JetAlgorithmFirmwareImp1::chunkyDonutPUEstimate(l1t::Jet& jet,
  ...
  for (unsigned int i = 0; i < 4; ++i)
    jet.setPUDonutEt(i, (short int)ring[i]);
  ...
  if (params_->jetCalibrationType() == "function6PtParams22EtaBins") {  //One eta bin per region
    ...
    if (params_->jetCalibrationParams().size() != 6 * 22) {
      edm::LogError("l1t|stage 2") << "Invalid input vector to calo params. Input vector of size: "
                                   << params_->jetCalibrationParams().size()
    ...
    if (params_->jetCalibrationParams().size() != 8 * 22) {
      edm::LogError("l1t|stage 2") << "Invalid input vector to calo params. Input vector of size: "
                                   << params_->jetCalibrationParams().size()
                                   << "  Require size: 176  Not calibrating Stage 2 Jets" << std::endl;
      return;
    }
    ...
    if (params_->jetCalibrationType() != "None" && params_->jetCalibrationType() != "none")
      edm::LogError("l1t|stage 2") << "Invalid calibration type in calo params. Not calibrating Stage 2 Jets"
                                   << std::endl;
    return;
  }
}
...
double l1t::Stage2Layer2JetAlgorithmFirmwareImp1::calibFitErr(double pt, double* par) {
  double f = par[0] + par[1] * TMath::Erf(par[2] * (log10(pt) - par[3]) +
                                          par[4] * exp(par[5] * (log10(pt) - par[6]) * (log10(pt) - par[6])));
  // sanity check
  if (f < 0)
    f = 0;
  if (f != f)  // stop NaN
