...
namespace l1t {
  namespace stage2 {
    bool CaloTowerUnpacker::unpack(const Block& block, UnpackerCollections* coll) {
      ...
      int nBX = int(ceil(
          ...
          nframes));  // Since there are two Rx links per block with 2*28 slices in barrel and endcap + 2*13 for upgraded HF
      ...
      LogDebug("L1T") << "Block : id=" << block.header().getID() << ", size=" << block.header().getSize()
                      << ", link=" << link << ", link_phi=" << link_phi << ", nBX=" << nBX << ", firstBX=" << firstBX
                      << ", lastBX=" << lastBX;
      ...
      for (int bx = firstBX; bx <= lastBX; bx++) {
	    ...
	    std::cout << ((raw_data >> 12) & 0xF) << " = tower1.setHwQual((raw_data >> 12) & 0xF) in CaloTowerUnpacker.cc" << std::endl;
            ...
            LogDebug("L1T") << "Tower 1: Eta " << tower1.hwEta() << " phi " << tower1.hwPhi() << " pT " << tower1.hwPt()
                            << " frame " << iframe << " qual " << tower1.hwQual() << " EtRatio " << tower1.hwEtRatio();
	    ...
	    std::cout << ((raw_data >> 28) & 0xF) << " = tower2.setHwQual((raw_data >> 28) & 0xF) in CaloTowerUnpacker.cc" << std::endl;
            ...
            LogDebug("L1T") << "Tower 2: Eta " << tower2.hwEta() << " phi " << tower2.hwPhi() << " pT " << tower2.hwPt()
                            << " frame " << iframe << " qual " << tower2.hwQual() << " EtRatio " << tower2.hwEtRatio();
