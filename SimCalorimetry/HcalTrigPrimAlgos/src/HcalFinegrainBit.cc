...
std::bitset<6> HcalFinegrainBit::compute(const HcalFinegrainBit::TowerTDC& tower, const HcalTrigTowerDetId& id) const {
  ...
  //  if ((result[0] == 1) | (result[1] == 1) | (result[2] == 1) | (result[3] == 1) | (result[4] == 1) | (result[5] == 1)) std::cout << result[0] << " = depth flagged; " << result[1] << " = prompt; " << result[2] << ", " << result[3] << " = slightly delayed, very delayed; " << result[4] << ", " << result[5] << " = MIP bits, for ieta, iphi = " << id.ieta() << ", " << id.iphi() << std::endl;
