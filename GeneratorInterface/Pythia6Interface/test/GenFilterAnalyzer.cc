...
class GenFilterAnalyzer : public edm::one::EDAnalyzer<edm::one::WatchRuns> {
  public:
...
void
GenFilterAnalyzer::analyze(const edm::Event& event, const edm::EventSetup& iSetup){
...
//    if(verbose_) printf("%4d: ID=%4d, mom (index,id)=(%4d,%4d), mass=%3.1f, pt=%4.1f, eta=%4.1f, status=%3d\n",index,pdgId,momInd,momId,mass,pt,eta,status);
    if( abs(pdgId)==5 ){ // b quarks
      genbs_allP.emplace_back(it);
//      if( abs(momId)==36 )// decay from A0 particle
      num_b_allP ++;
    }

    if( (abs(pdgId)==11 || abs(pdgId)==13) && status==1 ) {
      num_lep_allP ++;
      if(pt>10 && abs(eta)<2.5){
        num_lep_pt10_allP ++;
        h_momid_lep_allP->Fill(abs(momId));
        h_pt_lep_allP->Fill(pt);
        h_energy_lep_allP->Fill(energy);
        h_eta_lep_allP->Fill(eta);
        h_phi_lep_allP->Fill(phi);
      }
//      printf("Gen Leptons: ID=%d, momId=%d, mass=%5.1f, pt=%6.1f,energy=%6.1f,status=%d\n",pdgId,momId,mass,pt,energy,status);
      if( find(MesonIDs.begin(), MesonIDs.end(), abs(momId)) != MesonIDs.end() ){
        h_pt_lep_from_meson->Fill(pt);       
        h_eta_lep_from_meson->Fill(eta);       
      }
      else if( find(BaryonIDs.begin(), BaryonIDs.end(), abs(momId)) != BaryonIDs.end() ){
        h_pt_lep_from_baryon->Fill(pt);
        h_eta_lep_from_baryon->Fill(eta);
      }
      else if( abs(momId)==24 || abs(momId)==15 ){
        h_pt_lep_from_WTau->Fill(pt);
        h_eta_lep_from_WTau->Fill(eta);
      }

    }

    // require hard process //
...
//      if( abs(momId)==36 )// decay from A0 particle
    ...
    //else if( abs(pdgId)==11 || abs(pdgId)==13 || abs(pdgId)==15 ){ // leptons
    if( (abs(pdgId)==11 || abs(pdgId)==13) ){ // leptons
      num_lepton ++;
      h_pt_lep->Fill(pt);
      h_energy_lep->Fill(energy);
      h_eta_lep->Fill(eta);
      h_phi_lep->Fill(phi);
      h_momid_lep->Fill(abs(momId));
      if(pt>10 && abs(eta)<2.5) num_lepton_pt10 ++;
        if (verbose_){
          printf("\n\n Lepton Gen Particles: \n");
          printf("Gen Particles: ID=%d, momId=%d, mass=%5.1f, pt=%6.1f,energy=%6.1f,status=%d\n",pdgId,momId,mass,pt,energy,status);
        }
    }
  }
  sort(genbs_allP.begin(), genbs_allP.end(), ptsort);
  int ib_allP = 0;
...
const reco::Candidate* GenFilterAnalyzer::findFirstMotherWithDifferentID(const reco::Candidate *particle){
  if( particle == 0 ) {
...
void 
GenFilterAnalyzer::beginJob() 
{
}

void 
GenFilterAnalyzer::beginRun(edm::Run const& iRun, edm::EventSetup const&){
}

void 
GenFilterAnalyzer::endJob() 
{
}

void
GenFilterAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
